...
const AddMeasure = props => {
  ...
  const handleChange = ({target: {id, value}}) => {
     // preventDefault();
      ...
      if (id === 'measure-input') {
        let stringNum = `${value} cm`
        setSelection({
          bodypart_id: selection.bodypart_id, name: selection.name, value: stringNum, date: selection.date,
        });
      }
      ...
      <main>
        ...
        <section className="d-flex justify-contet-center my-1">
          ...
          <form onSubmit={handleSubmit} className="flex-column d-flex align-items-center">
            ...
            <h4>how much?:</h4>
            <input type="number" placeholder="add new measure" id="measure-input" onChange={handleChange} required />
            <h4>date:</h4>
